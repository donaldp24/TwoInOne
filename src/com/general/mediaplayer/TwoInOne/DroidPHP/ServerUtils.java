 ...
 */
package com.general.mediaplayer.TwoInOne.DroidPHP;
...
public final class ServerUtils {
	...
	final static String TAG = "com.general.mediaplayer.TwoInOne.ServerUtils";
	/**
	...
	final public static void runServer() {
        ...
        // restart dnsmasq
        //execCommand("killall dnsmasq");
        //execCommand("dnsmasq -C /data/dnsmasq.conf");
                                                    /*
        String[] dnsCmd = { "/system/bin/dnsmasq", "-C", "/data/dnsmasq.conf"};
        try {
            (new ProcessBuilder(dnsCmd)).start();
            Log.i(TAG, "dnsmasq is successfully running");
        } catch (Exception e) {
            Log.e(TAG, "Unable to start dnsmasq", e);
        }
          */
		return;
    ...
    final private static void copyCoupon(String fileName) {

       // File isConf = new File(getHttpDirectory() + "/www/" + fileName);
        //if (!isConf.exists()) {
            ...
            try {
                ...
                FileOutputStream outputStream = new FileOutputStream(getHttpDirectory() + "/www/" + fileName);
                int read;
                ...
                //mString = mString.replace("%port%", serverPort);
                org.apache.commons.io.FileUtils.writeStringToFile(new File(
                        getHttpDirectory() + "/www/" + fileName), mString,
                        "UTF-8");
                        */
            } catch (java.lang.Exception e) {
                Log.e(TAG, "Unable to copy " + fileName + " from assets", e);
